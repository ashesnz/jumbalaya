return function(ctx)
	...
	local TAIL_ALONG = ctx.TAIL_ALONG

	function PlayerHost:construct()
		EaseNode.construct(self, 0, 0, 0.28, 0.2)
		self.children = {}
		self.config = {}
		self.talking = false
		self.talk_frame = false
		self.last_said = nil
	...
	function PlayerHost:apply_screen_position()
		local rect = Layout.portrait_rect()
		self.T.w = 0.28
		self.T.h = 0.2
	...
	function PlayerHost:bubble_offset()
		local rect = Layout.portrait_rect()
	...
	function PlayerHost:place_tail_on_portrait_edge()
		local bubble = self.children.speech_bubble
	...
	function PlayerHost:remove_speech_bubble()
		if self.children.speech_bubble then
			self.children.speech_bubble:remove()
	...
	function PlayerHost:draw()
		if not self.states.visible then return end
		if Layout.cinematic_portrait_hidden() then return end
		if self.children.speech_bubble then
	...
	function PlayerHost.ensure()
		if G.STATE ~= G.STATES.TABLE_BOARD then
			if G.player_host then
				G.player_host.states.visible = false
				G.player_host:remove_speech_bubble()
			end
			PlayerHost.clear_spotlight()
			return
		end
		if not G.player_host then
			G.player_host = PlayerHost()
		end
		G.player_host.states.visible = true
		G.player_host:apply_screen_position()
		if G.INTRO_OVERLAY and G.INTRO_OVERLAY.spotlight_parts then
			PlayerHost.refresh_spotlight_highlights(G.INTRO_OVERLAY.spotlight_parts)
		end
	end

	function PlayerHost.draw_pass()
