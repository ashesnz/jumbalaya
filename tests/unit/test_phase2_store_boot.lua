...
T.describe("Phase 2 Store Integration", function()
	...
	T.it("wires store in mock_env and binds WORD_GAME", function()
		T.assert_not_nil(G._store)
		T.assert_equal(word_game.store(), G._store)
	...
	T.it("dual-writes round init and start_hand via dispatch", function()
		...
		T.assert_not_nil(G._store:get().word_round)
		T.assert_equal(G.GAME.word_round.set, G._store:get().word_round.set)
		T.assert_equal(G.GAME.word_round.hand_index, G._store:get().word_round.hand_index)

		word_game.Round.start_hand(1, 2)
		T.assert_equal(G.GAME.word_round.hand_index, 2)
		T.assert_equal(G._store:get().word_round.hand_index, 2)
	end)
	...
	T.it("dispatch mirrors token changes onto G.GAME", function()
		...
		T.assert_equal(G._store:get().run_state.tokens, 10)
	end)
	...
	T.it("marks trade used via store dispatch", function()
		...
		T.assert_true(G._store:get().run_state.trade_used_this_hand)
	end)
	...
	T.it("dual-writes placement preview and busy flags via game_access", function()
		...
		T.assert_equal(G._store:get().placement_word, "")

		busy.set("trade_ui_busy", true)
		T.assert_true(G.GAME.trade_ui_busy)
		T.assert_true(G._store:get().trade_ui_busy)

		busy.clear()
		T.assert_nil(G.GAME.trade_ui_busy)
		T.assert_nil(G._store:get().trade_ui_busy)
	end)
	...
	T.it("dual-writes timeline fuse state via game_access mutate", function()
		...
		T.assert_equal(G._store:get().timeline_seconds, 45)
	end)
end)
