--[[ packages/jumbalaya_core/jumble/puzzle_spec.lua - Pattern parsing and matching (no G) ]]
...
function M.normalize_puzzle(row)
...
function M.validate_puzzle(puzzle)
...
---@param opts table|nil { is_valid_word: fun(word: string): boolean }
function M.word_fits_pattern(word, puzzle, opts)
	opts = opts or {}
	local is_valid = opts.is_valid_word
	...
	if puzzle.kind == "span" then
		...
		if puzzle.center then
			local start, finish = M.center_block_range(word, puzzle)
			if not start or word:sub(start, finish) ~= puzzle.center then
				return false
			end
		end
		return is_valid and is_valid(word) or false
	end
...
function M.random_boss_puzzle(words, opts)
	opts = opts or {}
	if not words or #words == 0 then return nil end
	local pick_word = opts.pick_word or function(ws)
		return ws[math.random(#ws)]
	end
	local pick_index = opts.pick_index or function(max)
		return math.random(max)
	end
	local word = pick_word(words)
	if not word then return nil end
	local revealed = {}
	local revealed_count = 0
	while revealed_count < 2 do
		local index = pick_index(#word)
