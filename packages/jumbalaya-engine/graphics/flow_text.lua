...
function FlowText:construct(config)
	...
	self.colours = config.colours or {game().C.RED}
	self.shown_at = game().TIMERS.REAL
...
function FlowText:pop_out(pop_out_timer)
	...
	self.fade_started_at = game().TIMERS.REAL + (self.pop_delay or 0)
end
	...
	for k, letter in ipairs(focused.letters) do
		...
		elseif self.config.pop_in then
			...
			if prev_pop_in <= 0 and letter.pop_in > 0 and not self.silent
				...
				if not (self.T.x > game().ROOM.T.w + 2 or self.T.y > game().ROOM.T.h + 2
...
function FlowText:draw()
	...
	if self.shadow then
		...
		for _, letter in ipairs(focused.letters) do
			...
			love.graphics.translate(letter.dims.x * self.font.FONTSCALE / game().TILESIZE, 0)
		end
		love.graphics.pop()
	end
