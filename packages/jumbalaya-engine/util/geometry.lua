--[[
	jumbalaya-engine/util/geometry.lua - flat point math shared by engine objects.
	...
	Prefer `local Geometry = require("jumbalaya-engine.util.geometry")` in new code.
	Boot calls `Geometry.install()` for legacy global aliases.
]]
...
---@return number
function M.point_distance(a, b, compare_centres)
...
function M.shift_point(point, offset)
...
function M.rotate_point(point, angle)
...
function M.install()
	_G.point_distance = M.point_distance
	_G.shift_point = M.shift_point
	_G.rotate_point = M.rotate_point
end
