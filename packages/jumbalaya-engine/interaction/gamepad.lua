...
function InputRouter:set_gamepad(_gamepad)
	...
	if self.GAMEPAD_CONSOLE ~= detected then
		...
		for _, v in pairs(game().LIVE.SPRITE) do
			if v.atlas == game().TEXTURE_ATLASES["gamepad_ui"] then
				v.sprite_pos.y =
					detected == 'Nintendo' and 2
					or detected == 'Playstation' and (game().F_PS4_PLAYSTATION_GLYPHS and 3 or 1)
					or 0
