...
function InputRouter:queue_R_cursor_press(x, y)
	...
	if not game().SETTINGS.paused and hand and hand.selected[1] then
...
function InputRouter:L_cursor_press(x, y)
	...
	self.press_state.time = game().TIMERS.TOTAL
	...
	if self.press_state.target == nil then
		self.press_state.target = game().ROOM
	end
end
...
function InputRouter:L_cursor_release(x, y)
	...
	self.release_state.time = game().TIMERS.TOTAL
	...
	if self.release_state.target == nil then
		self.release_state.target = game().ROOM
	end
end
end
