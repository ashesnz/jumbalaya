--[[ word_game.board.bonus.gutter - Bonus stack layout and drag/snap geometry ]]
...
local function timeline_rect()
	...
	return { x = 0, y = 1, w = 6, h = 0.7 }
end

function M.stack_y_lift()
	local dim_ok, dim = pcall(require, "word_game.config.layout.dimensions")
	local tile = game().TILESIZE or (dim_ok and dim.TILESIZE) or 20
	local scale = game().TILESCALE or (dim_ok and dim.TILESCALE) or 1
...
function M.stack_layout()
	...
	local card_w = game().CARD_W or 1
	local card_h = game().CARD_H or 1.4
...
function M.point_in_stack(x, y)
	if not BonusStack.is_active() then return false end
	local layout = M.stack_layout()
	local count = math.max(1, #(BonusStack.cards() or {}))
...
function M.return_card(card)
	...
	if not BonusStack.contains(card) then
		if card.bonus_card then
			BonusStack.add_card(card)
		else
			return false
		end
	end
	if card.area then
		if game().pattern_row and card.area == game().pattern_row.area
			and game().pattern_row.on_remove_card then
