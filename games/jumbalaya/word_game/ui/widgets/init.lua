...
function M.text(str, scale, colour)
	return { n = game().UI.TEXT, config = { text = str or "", scale = scale or 0.4, colour = colour or game().C.UI.TEXT_LIGHT, shadow = true } }
end

function M.row(nodes, extra)
	extra = extra or {}
	return { n = game().UI.ROW, config = extra.config or { align = extra.align or "cm", padding = extra.padding or 0.05 }, nodes = nodes }
end

function M.col(nodes, extra)
	extra = extra or {}
	return { n = game().UI.COLUMN, config = extra.config or { align = extra.align or "cm", padding = extra.padding or 0.05 }, nodes = nodes }
end
