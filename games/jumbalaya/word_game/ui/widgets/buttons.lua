...
function alpha_button_font()
	if not button_font then
		...
		button_font = {
			FONT = ok and font or love.graphics.newFont(game().TILESIZE * 7),
...
function make_keyboard_key(key, binding)
	...
	return Components.button{ label = {key_label}, onClick = "key_button", ref_table = {key = key == 'back' and 'return' or key},
			...
			col = true, colour = game().C.GREY, focus_args = binding and {button = binding, orientation = (key == ' ' or key == 'back') and 'cr' or 'bm', set_button_pip= true} or nil}
end
...
function make_bind_pip(args)
	...
	local button_sprite_map = {
		['a'] = game().F_SWAP_AB_PIPS and 1 or 0,
		['b'] = game().F_SWAP_AB_PIPS and 0 or 1,
		['x'] = 2,
		['y'] = 3,
		['leftshoulder'] = 4,
		['rightshoulder'] = 5,
	...
	local BUTTON_SPRITE = Sprite(0,0,args.scale or 0.45,args.scale or 0.45,game().TEXTURE_ATLASES["gamepad_ui"],
