...
function M.draw(timer, layout)
	...
	if shake > 0 then
		love.graphics.push()
		local real_time = (game().TIMERS and game().TIMERS.REAL) or 0
	...
	if font then
		...
		if timer.is_progress_mode() and (timer.goal_reached or timer.frozen_for_reward) then
			local pulse = math.abs(math.sin(((game().TIMERS and game().TIMERS.REAL) or 0) * 6))
		...
		elseif not timer.is_progress_mode() and timer.time_remaining <= 10 and timer.time_remaining > 0 then
			local pulse_red = math.abs(math.sin(((game().TIMERS and game().TIMERS.REAL) or 0) * 8))
