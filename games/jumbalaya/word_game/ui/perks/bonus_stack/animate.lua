...
function M.animate_cards_to_stack(queue_event, _easing_mod, opts)
	opts = opts or {}
	local S = stack_ui
	local cards = bonus_stack_model().cards() or {}
	...
	bonus_stack_model().set_animating(true)
	...
	local can_queue = queue_event and game().TIMELINE and game().TIMELINE.enqueue
