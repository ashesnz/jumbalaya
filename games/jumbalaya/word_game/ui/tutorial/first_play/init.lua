...
function M.is_active()
	return session.is_active_flag() and game().FIRST_PLAY_TUTORIAL_OVERLAY ~= nil
end

function M.should_show()
	if game() and game().F_SKIP_TUTORIAL then return false end
...
function M.dismiss()
	if not session.is_active_flag() and not game().FIRST_PLAY_TUTORIAL_OVERLAY then return end
...
function M.begin()
	...
	if not game().ROOM_ATTACH then return false end
	...
	game().FIRST_PLAY_TUTORIAL_OVERLAY.flop_overlay = true
...
function M.try_schedule()
	...
	if game().STATE ~= game().STATES.TABLE_BOARD or game().STAGE ~= game().STAGES.RUN then return end
	...
	Scheduler.add{
		...
		blockable = false,
		func = function()
			if game().STATE == game().STATES.TABLE_BOARD and game().STAGE == game().STAGES.RUN then
				M.begin()
			end
			return true
		end,
...
function M.set_force(enabled)
	...
	if enabled and game().STATE == game().STATES.TABLE_BOARD then
		M.begin()
