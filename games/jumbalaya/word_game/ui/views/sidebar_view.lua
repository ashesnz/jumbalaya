...
local function tile_scale()
	return (game().TILESCALE or 1) * (game().TILESIZE or 20)
end
...
local function make_text_proxy(id, rect, text)
	local proxy = {
		config = {
			...
			colour = (game() and game().C and game().C.UI and game().C.UI.TEXT_LIGHT) or { 1, 1, 1, 1 },
			shadow = true,
		},
...
local function draw_panel(rect)
	local colour = (game() and game().C and game().C.DYN_UI and game().C.DYN_UI.MAIN) or { 0.22, 0.32, 0.35, 1 }
...
local function draw_counter_row(rect, count)
	...
	local colour = (game() and game().C and game().C.UI and game().C.UI.TEXT_LIGHT) or { 1, 1, 1, 1 }
	...
	love.graphics.setColor(colour)
	love.graphics.print(label, x, y, 0, scale * (game().TILESCALE or 1))
	love.graphics.print(tostring(count), x + label_w, y, 0, scale * (game().TILESCALE or 1))
end

local function draw_sidebar_deck(deck_rect)
	if not deck_rect or not game().draw_pile or not TableDeck.uses_table_draw() then return end
...
function SidebarView:draw(renderer)
	if not game() or game().STAGE ~= game().STAGES.RUN then return end
