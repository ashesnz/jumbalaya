...
T.describe("Platform and Globals Initialization", function()
	...
	T.it("initializes cleanly on iOS without nil SETTINGS indexing", function()
		...
		T.assert_nil(game.SETTINGS.GRAPHICS.crt, "CRT post-processing setting should be removed entirely")
	...
	T.it("initializes desktop settings on Windows and OS X", function()
		...
		T.assert_equal(game_win.F_CRASH_REPORTS, true, "Crash report setting should be available on Windows")
		...
		T.assert_equal(game_osx.F_CRASH_REPORTS, true, "Crash report setting should be available on OS X")
	end)
