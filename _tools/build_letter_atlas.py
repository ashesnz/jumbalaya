...
def sample_deck_face_colors(scale: str) -> dict[str, tuple[float, float, float, float]]:
    ...
    black_cell = deck.crop((0, 3 * cell_h, cell_w, 4 * cell_h))
    black_bg = sample_background(black_cell)
    return {
        "red": DECK_RED_RGBA,
        "black": sample_face_fill(black_cell, black_bg),
...
def write_face_colors_lua(colors: dict[str, tuple[float, float, float, float]]) -> None:
    ...
    lines = [
        ...
        "-- Default red is fixed at #7e1011; modified is #16151a; black is sampled from JumbalayaDeck.",
        "return {",
    ]
